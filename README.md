...
The AWS-SDK is, by default, included in ChatJS as a "baked-in" dependency. Version 2.x is consumed from [npmjs.com](https://npmjs.com/). In `./client/client.js` we import `ConnectParticipant` from this package. This dependency and import can be removed while using the AWS SDK imported through a script in the page file of your application, assuming that version of the AWS SDK has the `ConnectParticipant` service included.
